...
class RestartNodeManager(domain: Domain) extends Command {

  def result(): Result = {
    Chain(
      new StopNodeManager(domain),
      new StartNodeManager(domain)
    ).result
  }
}

object RestartNodeManager {
  def apply(domain: Domain, host: String = "localhost"): Command = {
    DoomAPI.remote(new RestartNodeManager(domain), host)
  }
}
