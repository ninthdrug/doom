...
class CheckQueueConsumerCount(
  ...
  def result() : Result = {
    ...
    try {
      ...
      val count = jmx.getQueueConsumerCount(queue)
...
object CheckQueueConsumerCount {
  def apply(queue: String, host: String, port: Int, user: String, password: String) =
    new CheckQueueConsumerCount(queue, host, port, user, password)
}
