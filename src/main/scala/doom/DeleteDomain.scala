...
/**
 * Command to delete a weblogic domain.
 */
class DeleteDomain(domain: Domain) extends Command {

  def result(): Result = {

    val cmd = Chain(
      Exec("rm -rf " + domain.domain_home),
      Chain(domain.machinenames map {
        m => Ssh("rm -rf " + domain.domain_home, m)
      })
    )
    cmd.result
  }
...
object DeleteDomain {
  def apply(domain: Domain): DeleteDomain = new DeleteDomain(domain)
}
