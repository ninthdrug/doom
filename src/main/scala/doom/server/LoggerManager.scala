...
object LoggerManager extends Actor {
  private val log = Logger("doom.server.LoggerManager")
   
  def act() {
    log.info("started")
    loop {
      react {
        case LoggerRequest(obj) => {
          if (obj.isInstanceOf[WorkOrder]) {
            val order = obj.asInstanceOf[WorkOrder]
            AuditCache.log(order)
          }
        }
      }
    }
